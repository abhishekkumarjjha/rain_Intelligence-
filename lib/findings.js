...
/** Lower-case mid-sentence, but never an acronym: "apr" reads as a typo. */
 ...
 */
export function buildFindings({ client, competitors, product, coverage, previous = null, ratePages = null, reference = [] }) {
  ...
  /**
   * HOW MUCH DIAGNOSTIC WEIGHT A FINDING CARRIES.
   *
   * "One competitor prints a $5 minimum" and "two of three use member-owned
   * positioning" were rendering identically, which told the reader they mattered
   * equally. They do not: one is a tactic belonging to a single advertiser, the
   * other is how most of the local set is talking.
   *
   * A rule over the count and denominator each finding already carries. No model
   * and no judgement — a share is a share.
   *
   *   primary     the client's stance on the product's headline rate, or a
   *               pattern at least half the readable set shares
   *   supporting  real, shared by more than one, short of a majority
   *   isolated    exactly one advertiser, out of three or more. A tactic.
   *   internal    about the client's own advertising, not a competitor's
   */
  const significanceOf = (f) => {
    if (f.outcome === OUTCOMES.CONTEXT) return "internal";
    if (f.metric && f.metric === profile.primaryRate) return "primary";
    const n = Number(f.count), d = Number(f.denominator);
    if (!Number.isFinite(n) || !Number.isFinite(d) || d <= 0) return "supporting";
    if (n / d >= 0.5) return "primary";
    // WHO the single advertiser is decides what "one of four" means.
    //
    // A lone COMPETITOR printing a $5 minimum is a tactic the client can
    // ignore, and that is what this demotion was built for. The CLIENT being
    // the only one printing a rate discount is the opposite: being alone in it
    // IS the finding, and it is the entire reason the lead column exists.
    // Treating the two the same filed every sole advantage under "other
    // differences" and left "Where you lead: 0" standing directly above two
    // cards naming exactly where they led.
    //
    // At two competitors, "one of them" is half the market, not an outlier.
    if (f.outcome !== OUTCOMES.LEAD && n === 1 && d >= 3) return "isolated";
    return "supporting";
  };

  // A STABLE ID PER FINDING — rule plus what it is about.
  //
  // "rate_advantage_apy", "claim_gap_member_owned". Deterministic across runs,
  // so a report line, an evidence chip and a month-over-month delta can all
  // refer to the same finding and mean it.
  const idFor = (f) => [f.rule, f.metric || f.claimId || ""].filter(Boolean).join("_");

  const push = (f) => {
    if (f) out.push({
      // WHICH POPULATION THIS WAS COUNTED OVER.
      //
      // Everything in this file except the national block below is counted over
      // LOCAL competitors, and that has always been true — but the board never
      // said so, and "Competitive pressure: 0" therefore read as "nobody is
      // ahead of you" when what it meant was "no LOCAL competitor is ahead".
      // On the La Capitol credit-card board both national advertisers led on a
      // cash bonus the client does not advertise, and the page said nothing.
      scope: "regional",
      ...f,
      id: f.id || idFor(f),
      chip: f.chip || chipFor(f),
      significance: f.significance || significanceOf(f),
      priority: PRIORITY[f.rule] ?? 0,
    });
  };
  ...
  for (const metric of profile.metrics) {
      ...
      if (D >= 1) {
        ...
        push({
          ...
          headline: positive
            ...
            : `${client.label}'s ads print ${soften(m.label)} of ${subject.raw}. No competitor's captured ads printed one.`,
          detail: positive
          ...
          // NOTHING WAS RANKED HERE. This branch fires precisely because no
          // competitor printed this figure at all, so `denominator` is the
          // number of BRANDS in the comparison (the client plus D
          // competitors) and not a count of comparable competitors — which
          // is what it means in the ranked branch below. Two meanings on one
          // field is P1, and the primary read duly picked the wrong one and
          // announced "the strongest APY of the 4 comparable local
          // competitors" over a set where four competitors printed nothing.
          // Stated as a flag so no reader has to infer it from the numbers.
          sole: true,
          ...
          reportLine: positive
            ...
            : `${client.label}'s captured ads showed ${soften(m.label)} of ${subject.raw}; ${soften(m.label)} was not observed in ${nOf(D, "selected competitor")}' captured ads.`,
        });
      }
      continue;
    }
    ...
    push({
      ...
      reportLine: rank.stronger.length === 0
        ? `Among ${rank.comparableCount} comparable competitors' captured ${soften(profile.label)} ads, none advertised ${strongerWord(m)} ${m.label} than ${client.label}'s ${subject.raw}.`
        : `Among ${rank.comparableCount} comparable competitors' captured ${soften(profile.label)} ads, ${rank.stronger.length} advertised ${strongerWord(m)} ${m.label} than ${client.label}'s ${subject.raw}${rank.weaker.length ? ` and ${rank.weaker.length} advertised ${weakerWord(m)}` : ""}.`,
    });
  }
  ...
  // ---- CLAIMS --------------------------------------------------------------
  // Claims the client alone advertises, collected across the loop and reported
  // as one finding rather than one card per noun.
  const soleClaims = [];
  for (const claimId of Object.keys(profile.claims)) {
    ...
    // COLLECTED, not pushed one at a time — see below.
    if (clientHas && withClaim.length === 0 && D >= 1) soleClaims.push(claimId);
  }

  // ---- ONE CARD FOR THE CLIENT'S SOLE MESSAGES ----------------------------
  //
  // These were pushed per claim, so a client saying three things nobody else
  // said produced three cards of the same sentence with one noun changed:
  //
  //   "…is the only advertiser in the captured set mentioning cash back."
  //   "…is the only advertiser in the captured set mentioning travel rewards."
  //   "…is the only advertiser in the captured set mentioning open online."
  //
  // That is one observation with three examples, and rendering it as three
  // findings inflates the count on a board that is explicitly never padded.
  if (soleClaims.length) {
    const names = soleClaims.map((id) => soften(claimLabel(id)));
    push({
      rule: "claim_advantage",
      claimId: soleClaims[0],
      label: "MESSAGE ADVANTAGE",
      unit: UNITS.BRAND,
      direction: "positive",
      outcome: OUTCOMES.LEAD,
      headline: soleClaims.length === 1
        ? `${client.label} is the only advertiser in the captured set mentioning ${names[0]}.`
        : `${client.label} is the only advertiser in the captured set mentioning ${listOf(names)}.`,
      detail: soleClaims.map((id) => `"${trim(client.claims.get(id).verbatim)}"`).join(" · ")
        + ` — not observed in any of ${D} competitors' captured ads.`,
      count: soleClaims.length, denominator: D + 1,
      evidence: dedupe(soleClaims.flatMap((id) => client.claims.get(id).evidence)),
      reportLine: `${client.label}'s captured ads mentioned ${listOf(names)}; ${
        soleClaims.length === 1 ? "this message was" : "these messages were"
      } not observed in ${nOf(D, "selected competitor")}' captured ads.`,
    });
  }
  ...
  // ---- NATIONAL REFERENCE --------------------------------------------------
  //
  // Nationals are excluded from every denominator and that stays true: we
  // cannot tell from the Transparency Center whether a Chase ad ever served in
  // Baton Rouge, so "3 of 5 competitors" must never quietly include them.
  //
  // But excluding them from the COUNT is not a reason to exclude them from the
  // PAGE. Both national advertisers leading on a bonus the client does not
  // advertise is a real thing a strategist should see, and burying it under a
  // scoreboard reading zero is its own kind of wrong answer.
  //
  // So: its own findings, its own scope tag, its own population named in every
  // sentence, and never a ratio that could be mistaken for a local one.
  const refUsable = (reference || []).filter((r) => r.hasCoverage);
  if (refUsable.length && client.hasCoverage) {
    for (const metric of profile.metrics) {
      const m = metricOf(metric);
      if (!m || m.direction === "none") continue;
      const withIt = refUsable.filter((r) => r.positions[metric]);
      // Only when EVERY captured national leads on it and the client does not.
      // One of two is a single advertiser's tactic, and a national's tactic is
      // the weakest evidence on the page.
      if (!withIt.length || withIt.length !== refUsable.length) continue;
      if (client.positions[metric]) continue;
      ...
      push({
        ...
        headline: `${withIt.length === 1 ? "The national reference advertiser" : `Both national reference advertisers`} advertise ${soften(m.label)}. ${client.label}'s captured ads do not.`,
        detail: `${withIt.map((r) => `${r.label} ${r.positions[metric].raw}`).join(" · ")} — national advertising, shown as context. We cannot tell from the Transparency Center whether it served in this market, so it is in no local count.`,
        count: withIt.length, denominator: withIt.length,
        evidence: dedupe(withIt.flatMap((r) => r.positions[metric].all.map((f) => f.creativeId))),
        reportLine: `${withIt.length === 1 ? "The national reference advertiser" : "Both national reference advertisers"} advertised ${soften(m.label)} in the captured ads; ${client.label}'s captured ads did not. National advertisers are reference context and are not counted among the selected local competitors.`,
      });
    }
  }
  ...
  if (client.hasCoverage) {
    for (const metric of profile.metrics) {
      ...
      // Both sides of a MIXED MESSAGE card are quoted back to the client, so
      // both have to be things the client actually printed. An ungrounded fact
      // or claim would manufacture the contradiction the card is reporting.
      const feeAd = (client.ads || []).find((a) => (a.facts || []).some((f) => f.metric === metric && f.grounded !== false));
      const freeAd = (client.ads || []).find((a) => (a.claims || []).some((c) => c.claim === negatingClaim && c.grounded !== false));
...
function gapFinding({ metric, m, others, D, client, coverage, profile }) {
  ...
  return {
    ...
    headline: ratio(others.length, D, coverage,
      `${others.length} of ${D} competitors advertise ${soften(m.label)}. ${client.label}'s captured ads do not.`,
      `${listNames(others)} ${verb(others.length, "advertises", "advertise")} ${soften(m.label)}; ${client.label}'s captured ads do not.`),
    ...
    reportLine: `${m.label} appeared in ${others.length} of ${D} selected competitors' captured ${soften(profile.label)} ads; ${client.label}'s captured ads did not advertise ${soften(m.label)}.`,
  };
}
  ...
  for (const brand of [client, ...usable]) {
    ...
    for (const metric of profile.metrics) {
      ...
      out.push({
        ...
        direction: "neutral",
        outcome: OUTCOMES.CONTEXT,
        headline: `${brand.label}'s captured ads advertise ${adv.raw}. Their current ${soften(profile.label)} rate page shows ${cur.raw}.`,
        ...
        sourceNote: page.url,
        reportLine: `${brand.label}'s captured ads advertised ${adv.raw}; their ${soften(profile.label)} rate page showed ${cur.raw} when fetched on ${page.fetchedAt?.slice(0, 10)}.`,
      });
    }
  }
  return out;
