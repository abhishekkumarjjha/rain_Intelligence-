...
// ---------------------------------------------------------------------------
// RE-READING ONE CREATIVE.
//
// Until now the only way to correct a wrong transcription was to bump the
// reader version, which retires EVERY extraction in the cache — hundreds of
// correct readings thrown away, and re-bought, to fix one. So in practice a
// wrong reading stayed wrong, and it stayed wrong in the evidence drawer where
// somebody had already noticed it.
//
// This is the cheap half of a human-correction workflow: forget one creative,
// and the next capture that meets it pays for one vision call and writes a new
// record. It is also what unblocks a golden corpus — a creative whose label a
// human has disputed has to be re-readable on its own.
//
// The EVIDENCE bundle is deliberately untouched. That archive is the answer to
// "prove they advertised this in August" and is never rewritten; forgetting a
// transcription must not quietly edit the record it was transcribed from.
// ---------------------------------------------------------------------------
export function forgetCachedExtraction(creativeId, reader = null) {
  const id = String(creativeId || "").replace(/[^a-zA-Z0-9_-]/g, "").slice(0, 80);
  if (!id) return { dropped: 0, readers: [] };
  const readers = [];
  try {
    for (const f of readdirSync(CACHE)) {
      if (!f.startsWith(`${id}.`) || !f.endsWith(".json")) continue;
      // <creativeId>.<reader>.json — a reader argument narrows it to one
      // reading; without one, every reader's reading of this creative goes,
      // which is what "re-read this creative" means to the person asking.
      const readerPart = f.slice(id.length + 1, -".json".length);
      if (reader && readerPart !== String(reader)) continue;
      unlinkSync(path.join(CACHE, f));
      readers.push(readerPart);
    }
  } catch { /* a cache that cannot be pruned is not a reason to fail a request */ }
  return { dropped: readers.length, readers };
}
 ...
 */
/**
 * Did these two runs ask the same question of the same stretch of time?
 *
 * `days` is written on every run at capture. A run from before that field
 * existed cannot establish its window and is refused rather than guessed at:
 * no delta is honest, a wrong delta is not.
 */
export function comparableWindow(candidate, run) {
  const a = Number(candidate?.days);
  const b = Number(run?.days);
  ...
  try {
    const candidates = readdirSync(ROOT)
      ...
      .filter((r) => r && r.id !== run.id
        ...
        // THE WINDOW IS PART OF COMPARABILITY, for the same reason it is part
        // of previousSnapshot()'s gate. A 30-day run diffed against a 90-day
        // one reports every creative the wider window newly reveals as "new
        // since the last capture" — nothing changed but the question asked.
        // F-020 fixed this for the board's snapshot and left the run-diff
        // strip, which is fed from here, with the identical defect.
        //
        // Fails closed: a candidate whose window cannot be established is not
        // offered. A day of tolerance absorbs a daylight-saving boundary,
        // where today-minus-30 is 29 or 31 days of wall clock; it is nowhere
        // near wide enough to let 30 meet 60.
        && comparableWindow(r, run)
