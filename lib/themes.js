...
//
// The client's own designs are handed to the model ON PURPOSE — the cohort
// contrast is not readable without them. But the framing line counted them
// into "the N distinct designs captured" while the panel's own readOver.designs
// counted competitors only, so one panel carried two different Ns for the same
// noun, and the larger one was presented as a competitor figure.
//
// The client is a third population on the Wall — out of every competitor tier,
// chip, cluster, denominator and funnel step. A count is not the one place that
// rule gets to lapse. So the sentence names the competitor count, and says
// plainly that the client's work was read alongside it: a reader who knows the
// client's designs were in the read can weigh a theme that leans on them.
// ---------------------------------------------------------------------------
export async function readThemes(families = [], productLabel = "", counts = {}) {
  ...
  // Named counts when the caller supplies them; otherwise everything handed in
  // is treated as competitor work, which is what a caller with no client to
  // separate is telling us.
  const clientDesigns = Number(counts.clientDesigns) || 0;
  const competitorDesigns = Number.isFinite(Number(counts.competitorDesigns))
    ? Number(counts.competitorDesigns)
    : Math.max(0, usable.length - clientDesigns);
  const clientLabelOr = (fallback) => String(counts.clientLabel || fallback);
  ...
  return { audit, themes: {
    ...
    framing: `Recurring ideas across the ${competitorDesigns} distinct ${
      productLabel ? `${productLabel.toLowerCase()} ` : ""
    }competitor image designs captured${
      clientDesigns ? `, read alongside ${clientDesigns} of ${clientLabelOr("the client")}\u2019s own` : ""
    } — one entry per design, however many sizes it was cut into. These describe what the ads are, not what anyone should do, and they cover what was captured rather than the whole market.`,
    // The COMPETITOR count, matching readOver.designs on the same panel. The
    // client's designs are reported separately rather than folded in.
    creativesRead: competitorDesigns,
    clientDesignsRead: clientDesigns,
    designsHandedToTheModel: usable.length,
  } };
}
