...
import { buildPrimaryRead } from "./primary-read.js";
...
export function buildBoard({
  ...
  // confident: true — this is the counting boundary. A creative the reader could
  // not place on this product is still captured, still on the wall and still in
  // the funnel; it simply never becomes a position, a claim or a denominator.
  const scope = (ads) => normalizeAll(filterByProduct(ads || [], product, { confident: true }));
  ...
  const competitorBrands = competitors.map((c) => {
    ...
    return {
      ...
      // What the provider LISTED for this advertiser. "Chase ran no auto loan
      // ads" and "we sampled 30 of Chase's 4,000 ads and none were auto loan"
      // are very different statements, and only the second is true.
      listedTotal: Number(progress?.[c.domain]?.listed ?? progress?.[c.domain]?.found) || 0,
    };
  });
  ...
  const all = buildFindings({
    ...
    ratePages,
    reference: referenceBrands,
  });

  const shown = topFindings(all, maxFindings);

  // Computed BEFORE the read, because the read quotes it: what the local set
  // competes on, and what the nationals do, are two of the three sentences a
  // salesperson needs at the top.
  const setShape = readSetShape({
  ...
  return {
    ...
    // THE ONE SENTENCE AT THE TOP — the same findings, arranged into a read
    // instead of a list. Counted in code, never written by a model, and never
    // a recommendation. See primary-read.js.
    primaryRead: buildPrimaryRead({ findings: all, client: clientBrand, competitors: usable, coverage, profile, setShape }),
...
const andList = (xs) => xs.join(", ").replace(/, ([^,]*)$/, " and $1");
...
function offerSnapshot({ profile, client, competitors, reference = [] }) {
  ...
  // ---- WHAT EACH ADVERTISER IS ACTUALLY LEADING WITH -----------------------
  //
  // The matrix was right for a rich product and wrong for a thin one. A checking
  // board fills four columns; an auto-loan board fills two and shows a grid of
  // "none captured" with three rows that contribute nothing at all — and it
  // DROPPED facts, because a column list fixed to the profile has nowhere to put
  // Neighbors' "Up to 100% Financing".
  //
  // So: a line per advertiser, carrying every figure they printed rather than
  // the ones the profile happened to name, plus what they message on. The
  // metric-by-metric grid still exists in the full table below — this answers
  // the different question of what each brand is competing on.
  const summarise = (b) => {
    const figures = Object.entries(b.positions || {}).map(([metric, p]) => {
      const note = qualifierNote(p);
      // A bare "84" is not a term. Where the verbatim carries no unit of its
      // own, the qualifier that spells it out stands in — the ad said
      // "84 months", and printing the digits alone loses what they measure.
      const bare = !/[%$a-z]/i.test(String(p.raw || ""));
      return {
        metric, label: metricOf(metric)?.label || metric,
        raw: bare && note ? note : p.raw,
        note: bare ? "" : note,
        evidence: p.all.map((f) => f.creativeId),
      };
    });
    const claims = [...(b.claims?.keys?.() || [])].map((c) => claimLabel(c));
    return {
      key: b.key, label: b.label, domain: b.domain,
      isClient: !!b.isClient, tier: b.tier || "local",
      adCount: b.adCount, figures, claims,
      // EVERY ADVERTISER IS CLICKABLE, not only the ones that printed a figure.
      //
      // Per-figure chips covered the rows with numbers and left the rest dead:
      // "Baton Rouge Telco — 1 credit card ad, no figure printed" is exactly the
      // row a strategist wants to open, because "no figure" is a claim about
      // their advertising and the ad is the only thing that can settle it.
      evidence: [...new Set((b.ads || []).map((a) => a.creativeId).filter(Boolean))],
      // The sentence itself, composed here so the client and the copy button
      // read the same words the screen does.
      text: figures.length
        ? `${nOf(b.adCount, `${scopeLabel} ad`)}. Advertised ${
            andList(figures.map((f) => f.raw))}.${claims.length ? ` Messages on ${andList(claims.slice(0, 3)).toLowerCase()}.` : ""}`
        : `${nOf(b.adCount, `${scopeLabel} ad`)}, no figure printed.${
            claims.length ? ` Competing on ${andList(claims.slice(0, 3)).toLowerCase()} rather than a rate.` : ""}`,
    };
  };

  const participating = [client, ...competitors].filter((b) => b.hasCoverage);
  // Everyone with nothing on this product, said ONCE rather than as a run of
  // empty rows. For a national the sample size is the point: "no auto loan ads"
  // reads as a fact about Chase when it is a fact about our thirty-ad sample.
  const absent = [...competitors, ...reference].filter((b) => !b.hasCoverage).map((b) => ({
    label: b.label, tier: b.tier || "local",
    text: b.capturedTotal
      ? `${b.label} — ${b.listedTotal > b.capturedTotal
          ? `${b.capturedTotal} of about ${b.listedTotal.toLocaleString()} listed ads were sampled and none were ${scopeLabel}`
          : `${nOf(b.capturedTotal, "ad")} captured, none ${scopeLabel}`}`
      : `${b.label} — no ads captured in this window`,
  }));

  return {
    columns,
    rows: [row(client), ...competitors.map(row)],
    summaries: participating.map(summarise),
    referenceSummaries: reference.filter((b) => b.hasCoverage).map(summarise),
    absent,
    ...
    referenceNote: reference.length
      ? "National advertisers, shown as reference context. They are never counted among the selected local competitors — we cannot tell from the Transparency Center whether their ads served in this market — but where every one of them advertises something the client does not, the board says so on a card tagged National reference."
      : "",
  };
}
