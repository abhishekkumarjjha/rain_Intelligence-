...
// =============================================================================
// META FALLBACK EXTRACTION
//
// A separate prompt from the banner extractor above, because the inputs are
// different artifacts read for a different reason.
//
// The Google extractor reads a rendered BANNER where every fact — headline,
// offer, brand — exists only as pixels. It is the sole source of truth and must
// transcribe everything.
//
// A Meta card arrives with machine-readable title, body, CTA and destination
// already attached. Roughly half of them classify from the URL path alone, at
// zero cost and with no chance of a misread digit. So vision here is a FALLBACK
// for the remainder, and it must never overwrite a fact the deterministic tiers
// already established — a model guess replacing a URL-derived product is a
// strictly worse answer that looks identical downstream.
// =============================================================================

const META_SYSTEM = `You are looking at ONE creative image from a Facebook or Instagram ad run by a US bank or credit union.

Structured text for this ad was already read from the ad platform. Your job is
ONLY to supply what that text could not: what is legible in the artwork itself,
and how the creative looks.

You are a TRANSCRIBER. Report what is visibly present. Never complete, correct,
improve or infer any text or number.

=== IF AN OFFER IS VISIBLE IN THE ARTWORK ===
Transcribe it EXACTLY as printed, character for character. Never round, never
convert, never restate 4.50% as 4.5%. Keep asterisks and footnote markers.
Transcribe a term, minimum or condition ONLY if it is visibly printed. When it
is not, leave it empty — an invented minimum deposit is the most damaging thing
you could produce, because these figures end up in a comparison a client reads.

=== PRODUCT ===
Classify into exactly one of:
${PRODUCT_CODES.join(", ")}
A creative that only shows a brand or a lifestyle scene with no product cue is
"other" at low confidence, and that is the correct answer. Being unsure is
useful; being confidently wrong is not.

=== VIDEO ===
Some of these are a single frame taken from a video. Describe only that frame.
Never describe motion, sound, or anything you infer happened before or after it.

Return ONLY this JSON, no prose:
{
  "headlineInArt": "text set into the artwork, verbatim, else \\"\\"",
  "offer": {
    "present": false,
    "type": "rate | bonus | discount | fee_waiver | term | other | none",
    "value": "exactly as printed",
    "unit": "APY | APR | USD | percent | none",
    "term": "", "minimum": "", "qualifier": ""
  },
  "product": "one code from the list above",
  "productConfidence": 0.0,
  "visualStyle": "photo | illustration | typographic | product-shot | mixed",
...
/**
 * Read the representative asset for messages the free tiers could not resolve.
 *
 * @param {Array} messages  deduped messages carrying a stored media buffer
 * @returns {{read: number, failed: number}}
 */
export async function extractMetaMessages(messages = [], opts = {}) {
  const concurrency = Math.max(1, Math.min(12, opts.concurrency || 6));
  let read = 0, failed = 0;
      ...
      try {
        const msg = await createWithRetry({
          ...
          messages: [{
            role: "user",
            content: [
              { type: "image", source: { type: "base64", media_type: m._mediaType || "image/jpeg", data: m._mediaData } },
              { type: "text", text: "Transcribe and classify this creative. Return the JSON described in your instructions and nothing else." },
            ],
          }],
        });
        ...
        // DETERMINISTIC FACTS WIN. The model fills gaps; it never overwrites a
        // product read off a URL path or an offer transcribed from provider
        // text. Provenance is recorded either way so an audit can tell which
        // tier produced which field.
        if (!m.product && a.product) {
          m.product = coerceProductCode(a.product);
          m.productConfidence = typeof a.productConfidence === "number"
            ? Math.max(0, Math.min(1, a.productConfidence)) : 0.5;
          m.productFrom = "vision";
        }

        const o = a.offer && typeof a.offer === "object" ? a.offer : {};
        if (!m.offer && o.present && clean(o.value)) {
          m.offer = {
            type: ["rate", "bonus", "discount", "fee_waiver", "term", "other"].includes(o.type) ? o.type : "other",
            value: clean(o.value),
            unit: ["APY", "APR", "USD", "percent", "none"].includes(o.unit) ? o.unit : "none",
            term: clean(o.term), minimum: clean(o.minimum), qualifier: clean(o.qualifier),
            numeric: parseOfferNumber(clean(o.value)),
            from: "vision",
          };
          m.offerFrom = "vision";
        ...
        // Craft attributes only ever come from vision — nothing else can see them.
        m.headlineInArt = clean(a.headlineInArt);
        m.visualStyle = ["photo", "illustration", "typographic", "product-shot", "mixed"].includes(a.visualStyle) ? a.visualStyle : "mixed";
        m.hasPeople = !!a.hasPeople;
        m.tone = clean(a.tone);
        m.legible = a.legible !== false;
        m.visionRead = true;
        read++;
      } catch (e) {
        if (e.code === "NO_API_KEY") throw e;
