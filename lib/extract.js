...
import { isGrounded, sourceTextOf } from "./observations.js";
 ...
 */
export function shape(a, img) {
  const o = a.offer && typeof a.offer === "object" ? a.offer : {};
  const offerPresent = !!o.present && !!clean(o.value);

  // GROUNDING, the display half. See observations.js.
  //
  // A search ad is checked figure by figure in observations.js; a display
  // creative never passes through that file, because its offer is a single
  // transcribed object rather than a facts[] array. It needs the same check for
  // the same reason: "4.59% APR" that the reader did not also transcribe into
  // the headline, subhead, CTA or allText was not read off the artwork, and a
  // vision read has MORE room to invent than a text one, not less.
  //
  // Kept and marked rather than dropped: the wall shows what was captured, and
  // a creative whose offer cannot be grounded is still a creative that ran.
  const transcribed = sourceTextOf({
    headline: clean(a.headline), subhead: clean(a.subhead),
    cta: clean(a.cta), brand: clean(a.brand), allText: clean(a.allText),
  });
  const offerGrounded = offerPresent && isGrounded(clean(o.value), transcribed);
  ...
  return {
    ...
    offer: offerPresent ? {
      ...
      grounded: offerGrounded,
      // The number exists only so the table can sort, and an ungrounded figure
      // must never sort against a transcribed one. Withholding it here is the
      // whole gate: strongestOffer(), the offer counts and the cluster key all
      // read `numeric`, and none of them has to learn about grounding.
      numeric: offerGrounded ? parseOfferNumber(clean(o.value)) : null,
    } : null,
