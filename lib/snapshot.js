 ...
 */
export function putSnapshot({ clientDomain, product, source, label, brands, competitorSet, windowStart, windowEnd, days = null, runId }) {
  try {
    ...
    const snapshot = {
      ...
      // THE WINDOW IS PART OF WHAT THIS SNAPSHOT MEANS, so it is recorded as a
      // number and not only as two dates. See previousSnapshot(): a 30-day
      // capture and a 90-day capture of the same client are not two readings
      // of the same thing, and diffing them reports the extra window as news.
...
/**
 * How many days this snapshot covered. Derived from its dates when the field
 * is absent, which is every snapshot written before that field existed —
 * those still carry windowStart and windowEnd, so almost nothing is lost.
 * null means the window genuinely cannot be established.
 */
export function snapshotWindowDays(snap) {
  if (Number.isFinite(Number(snap?.days))) return Number(snap.days);
  const a = Date.parse(snap?.windowStart), z = Date.parse(snap?.windowEnd);
  if (!Number.isFinite(a) || !Number.isFinite(z) || z < a) return null;
  return Math.round((z - a) / 86400000);
}

/**
 * @param days  the window the CURRENT run covered. When given, a snapshot
 *              taken over a different window is not offered as its previous.
 *
 * WHY THE WINDOW BELONGS IN THIS GATE.
 *
 * The gate was client + product + source. Run a 30-day benchmark in July and
 * a 90-day one in August and the August board picks up July's snapshot as its
 * previous — so every figure the wider window newly reveals is reported as
 * "newly observed since the July benchmark". Nothing changed in the market.
 * The window changed, and the tool called it news, in the most quotable
 * sentence it produces.
 *
 * Fails closed: a candidate whose window cannot be established at all is not
 * offered. No delta is honest; a wrong delta is not.
 *
 * A day of tolerance absorbs a window that straddles a daylight-saving
 * boundary, where today-minus-30 is 29 or 31 days of wall clock. It is not
 * wide enough to let 30 meet 60.
 */
