...
import { MAX_READ_PER_ADVERTISER } from "./atc-provider.js";
...
// ---------------------------------------------------------------------------
/* ---------------------------------------------------------------------------
   IS THERE AN AD IN THIS PICTURE AT ALL?

   Some captures are not creatives. The Transparency Center renders its own
   page furniture — the collapse/expand controls, the overflow menu — and when
   a creative fails to paint, that chrome is what the screenshot contains. The
   reader then dutifully transcribes it, and the wall shows a white card
   reading "Collapsed ad on mobile / Collapsed ad on desktop / Expanded ad /
   more_vert" under the headline "(no legible headline)".

   It sorts to the TOP, because it is old and has been "shown" for hundreds of
   days, so the first thing on the wall of competitor creative is a picture of
   Google's own UI.

   Two independent tests, because they catch different failures:
     · no headline AND no subhead — nothing to read, whatever the image holds
     · the chrome strings themselves — the ad may have transcribed a headline
       from the page furniture, which is worse than transcribing nothing

   These are DROPPED FROM THE WALL, not from the counts. The funnel still says
   how many creatives were read; `unreadable` says how many of them turned out
   to hold nothing, so the arithmetic on screen still reconciles.
   --------------------------------------------------------------------------- */
const CHROME = /\b(collapsed ad on (mobile|desktop)|expanded ad|more_vert|more_horiz|ad_?unit|about this ad)\b/i;

export function isShowable(ad) {
  if (!ad) return false;
  const head = String(ad.headline || "").trim();
  const sub = String(ad.subhead || "").trim();
  // Chrome in the headline is disqualifying on its own: a headline lifted from
  // the viewer's own buttons is not this advertiser's copy.
  if (head && CHROME.test(head)) return false;
  if (head || sub) return true;
  // No copy at all. An offer figure is still a finding worth showing; nothing
  // at all is a blank card.
  return !!ad.offer?.value;
}

export function clusterAds(ads) {
  const groups = new Map();
  for (const ad of ads) {
    // The identity of an idea: WHOSE it is, what it says, and what it offers.
    // Two banners from the same advertiser with the same headline and offer at
    // different pixel sizes are one idea sized for two placements.
    //
    // ADVERTISER IS PART OF THE KEY and must stay that way. Without it, two
    // different banks running the same generic line — "Open an account today",
    // "Rates as low as" — collapse into a single card credited to whichever ran
    // longer, and the other bank's evidence disappears from the wall entirely.
    // That is not a variation; it is two competitors making the same bet, which
    // is a finding. The failure is silent and it gets worse the more
    // competitors are added, because generic national copy overlaps with
    // everyone.
    const key = [ad.institution, norm(ad.headline), norm(ad.offer?.value || ""), ad.product].join("|");
...
//
// Both extractors compute productConfidence, clamp it to 0..1, and store it.
// Nothing read it. So a creative the reader placed on auto loans at 0.1 —
// which is the reader correctly telling us it could not tell — entered the
// auto-loan denominator beside one at 0.95, and "3 of 4 competitors advertise
// an auto loan" was built partly out of the reader's own uncertainty.
//
// The floor is 0.5, and the number is chosen rather than picked. 0.5 is exactly
// where shapeSearch() and shape() park a read that returned no confidence at
// all, so the floor refuses only creatives the model ACTIVELY marked as unsure
// and never one that simply omitted the field. Raising it above 0.5 would start
// silently discarding cached extractions from before the field was populated,
// which is the cache-shape failure this repo has already paid for once.
//
// The treatment is the clipped figure's: kept, captured, visible, uncounted.
// It stays in run.ads, in the funnel, on the wall and in the product chips —
// it really was captured — and it contributes nothing to any comparison.
export const PRODUCT_CONFIDENCE_FLOOR = 0.5;

/** Did the reader place this creative on a product firmly enough to count it? */
export function confidentlyClassified(ad) {
  const c = ad?.productConfidence;
  // Absent means an old record from before the field existed. Those were counted
  // before this gate and are not evidence of an unsure read, so they still count.
  return typeof c !== "number" || c >= PRODUCT_CONFIDENCE_FLOOR;
}

/**
 * @param confident  true at every COUNTING boundary — the board, the snapshot,
 *                   the audit table. False (the default) everywhere the answer
 *                   is "what did we capture": the wall, the chips, the funnel.
 */
export function filterByProduct(ads, product, { includeAdjacent = false, confident = false } = {}) {
  const sure = confident ? (a) => confidentlyClassified(a) : () => true;
  if (!product || product === "all") return confident ? ads.filter(sure) : ads;
  return ads.filter((a) => {
    if (!sure(a)) return false;
...
// ---------------------------------------------------------------------------
export function buildBenchmark({ client, competitors, product, runs, brands = null }) {
  // The SAME scope the board uses. Two aggregations over two different ad sets
  // is how one screen shows one number in a finding and another in the table
  // below it, and a confidence gate applied to only one of them would rebuild
  // that bug from scratch.
  const scope = (ads) => filterByProduct(ads || [], product, { confident: true });
  ...
  // ---- offer rows: ONE CANONICAL ROLLUP, NOT A SECOND OPINION -------------
  //
  // These rows used to compute their own "strongest" offer per brand, by
  // sorting on the LARGEST printed figure and deliberately refusing to consider
  // direction. The board next to them ranks by the metric's own direction. On
  // one auto-loan screen that produced Baton Rouge Telco at 4.84% APR in the
  // findings and 6.74% APR in this table — both defensible in isolation, and
  // together an invitation to trust neither.
  //
  // The table is the AUDIT TRAIL for the board. An audit trail that disagrees
  // with the thing it audits is worse than no audit trail, so the rows now read
  // the board's positions directly. `brands` is the rollup buildBoard already
  // produced; without it the offer rows are simply omitted rather than
  // reconstructed by a second rule.
  const byKey = new Map((brands || []).map((b) => [b.isClient ? "client" : b.domain, b]));
  const metricIds = [...new Set((brands || []).flatMap((b) => Object.keys(b.positions || {})))];

  for (const metric of metricIds) {
    const m = metricOf(metric);
    if (!m) continue;
    const cells = columns.map((col) => {
      const pos = byKey.get(col.key)?.positions?.[metric];
      if (!pos) return { column: col.key, value: "—", absent: true, evidence: [] };
      return {
        column: col.key,
        value: pos.raw,
        detail: qualifierLineFromPosition(pos),
        note: pos.adCount > 1 ? `${pos.adCount} ads` : "",
        evidence: (pos.all || []).map((f) => f.creativeId),
        primaryEvidence: pos.creativeId,
      };
    });
...
/** The qualifiers a board POSITION carries, in the table's own phrasing. */
function qualifierLineFromPosition(pos) {
  const q = pos.qualifiers || {};
  const bits = Object.entries(q).map(([k, v]) => {
...
// ---------------------------------------------------------------------------
export function captureFunnel(runs = [], ads = [], onProduct = null, counted = null) {
  ...
  const steps = [
    ...
    // The cap governs VISION CALLS, so duplicates are collapsed BEFORE it is
    // spent. Both losses land on this step and the wording names them in the
    // order they happen, rather than crediting the cap with all of it.
    { key: "selected", label: "selected to read", value: selected,
      lost: Math.max(0, renderable - selected),
      // Never open with a number: the UI prints this straight after the lost
      // count, and two figures running together read as one broken figure.
      why: duplicates
        ? `over the read cap of ${MAX_READ_HINT()} per advertiser, or duplicate renders of artwork already in hand`
        : `over the read cap of ${MAX_READ_HINT()} per advertiser` },
    { key: "read", label: "read", value: read,
      lost: Math.max(0, selected - read), why: "creatives the model could not transcribe, or whose download failed" },
  ];
  ...
  // ITS OWN STEP, not folded into the one above. A creative the reader could not
  // confidently place was NOT "classified as a different product" — it was
  // classified as this one, unsurely — and saying so is the difference between
  // a funnel that reconciles and a funnel that reconciles by lying about where
  // the drop went.
  if (onProduct != null && counted != null && counted !== onProduct) {
    steps.push({
      key: "counted", label: "counted in the comparison", value: counted,
      lost: Math.max(0, onProduct - counted),
      why: "the reader could not place them on this product with enough confidence to count",
    });
  }

  return {
    listed, retrieved, renderable, previewOnly, selected,
    duplicates, downloadFailed, read, onProduct, counted,
    steps: steps.filter((s) => s.value > 0 || s.lost > 0),
  };
}

// The cap actually in force, imported rather than restated. This function used
// to hold its own literal fallback, which silently went stale the moment the
// default moved from 18 to 30 — the funnel then reported a cap the code had
// stopped using. A number a user reads must have exactly one definition.
function MAX_READ_HINT() {
  return MAX_READ_PER_ADVERTISER;
}
