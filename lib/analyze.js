...
export function buildBenchmark({ client, competitors, product, runs }) {
  ...
  rows.push({
    ...
    cells: columns.map((col) => ({
      ...
      note: col.ads.length ? `${col.ads.length} executions` : "",
      evidence: col.ads.map((a) => a.creativeId),
    })),
  });
...
export function samplingNote(runs = []) {
  ...
  return {
    ...
    note: incomplete.length === 0
      ? `All ${total} creatives in this window were retrieved.`
      : `${read} of about ${total.toLocaleString()} creatives in this window were read. Findings describe the ads captured, not the whole market.`,
  };
}
