...
export function normalizeObservation(ad) {
  ...
  for (const f of ad.rawEconomicFacts || []) {
    ...
    facts.push({
      ...
      // Kept as evidence — the ad really does say it — but never the brand's
      // position on the rate, and never ranked against anyone else's rate.
      isReduction: m.unit === "percent" && RATE_REDUCTION.test(String(f.raw || "")),
      // AN AMOUNT YOU CAN BORROW IS NOT AN AMOUNT YOU RECEIVE. Baton Rouge
      // Telco's "Borrow Funds Up To $30,000*" was filed as a cash bonus and
      // ranked against Chase's $3,000 welcome offer. Both are dollars; only one
      // is a bonus. On a lending product a cash_bonus is the rarer reading, so
      // the amount has to be bound to bonus language to count as one.
      isLoanAmount: f.metric === "cash_bonus" && LOAN_AMOUNT.test(String(ad.allText || ""))
        && !BONUS_LANGUAGE.test(String(ad.allText || "")),
  ...
  for (const c of ad.rawClaims || []) {
    ...
    claims.push({ claim, verbatim: c.verbatim, sourceField: c.sourceField });
  }
...
export function rollUpBrand(column) {
  ...
  for (const [metric, list] of byMetric) {
    ...
    const complete = list.filter((f) => f.complete && !f.scopedToAddOn && !f.isReduction && !f.isLoanAmount);
    if (!complete.length) {
