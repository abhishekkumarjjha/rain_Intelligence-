...
/* How many creatives from ONE advertiser get read by the vision model.
   THE COST IS ONE VISION CALL PER CREATIVE — that is the only reason this cap
   exists. The listing itself is one cheap search credit for up to 100.
...
export const MAX_READ_PER_ADVERTISER = Number(process.env.RI_MAX_READ || 18);
...
export async function capture(domain, opts = {}) {
  ...
  const selected = selectForReading(listing.creatives, opts.max || MAX_READ_PER_ADVERTISER);
  const { images, downloadFailed, exactDupes } = await downloadCreatives(selected, normDomain(domain));
  ...
  return {
    ...
    run: {
      ...listing.run,
      selectedForReading: selected.length,
      downloaded: images.length + exactDupes,
      downloadFailed,
      exactDupes,
      // The chain any status line must be able to reconcile:
      //   providerTotal -> returned -> renderable -> selected -> downloaded -> read
      capped: listing.creatives.length > selected.length,
    },
  };
}
