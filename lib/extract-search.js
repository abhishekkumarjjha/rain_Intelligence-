...
A FEE FOR AN OPTIONAL ADD-ON IS NOT THE ACCOUNT'S MONTHLY FEE. Financial ads
routinely price a benefits bundle inside a product ad — "$5.99/month with
BaZing", "$3/month for identity protection". That figure is the cost of an
optional package, not what the account charges. When a fee is attached to a
named add-on or bundle, set "monthly_fee" with qualifiers.applies_to set to the
add-on's name. When the fee is the account's own, leave applies_to null. If you
cannot tell which it is, omit the fee entirely — a bundle price filed as an
account fee will be compared against competitors' account fees and will be
